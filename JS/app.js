...
function usernameFounder(event) {
  ...
  axios
    ...
    .then(function (response) {
      ...
      if (response.data.twitter_username == null) {
        ...
        twitterLinkElement.setAttribute(
          "href",
          `https://twitter.com/${response.data.twitter_username}`
        );
      }
    })
  ...
  axios.get(reposUrl).then(function (response) {
    let repositoryDetailsElement = document.querySelector(
      ".repository-details"
    );
    ...
    for (let i = 0; i < reposList.length; i++) {
      ...
      let reposTitleSelector = document.querySelector(
        `.repos-title${numberOfRepos}`
      );
      let reposParagraphSelector = document.querySelector(
        `.repos-paragraph${numberOfRepos}`
      );
      let topicParentElement = document.querySelector(
        `.repos${numberOfRepos} .topics`
      );
